...
def install_packages():
    ...
    render_put('deploy/apt/sources.list', '/etc/apt/sources.list.d/sources.list')
...
def github_config():
    ...
    render_put('deploy/.ssh/id_rsa', '~/.ssh/id_rsa')
    render_put('deploy/.ssh/id_rsa.pub', '~/.ssh/id_rsa.pub')
    render_put('deploy/.ssh/known_hosts', '~/.ssh/known_hosts')
...
def postgresql_setup():
    ...
    render_put('deploy/postgresql/pg_hba.conf', '/etc/postgresql/8.3/main/pg_hba.conf')
...
def install_project():
    ...
    render_put('deploy/_local_settings.py', '/webapp/%s/src/saaskit/local_settings.py' % env.host_string)
...
def nginx_config():
    ...
    render_put('deploy/nginx/assets', '/etc/nginx/sites-available/assets')
    sudo('ln -f -s /etc/nginx/sites-available/assets /etc/nginx/sites-enabled/assets', pty=True)
    render_put('deploy/nginx/webapp', '/etc/nginx/sites-available/webapp')
    sudo('ln -f -s /etc/nginx/sites-available/webapp /etc/nginx/sites-enabled/webapp', pty=True)
    render_put('deploy/nginx/nginx.conf', '/etc/nginx/nginx.conf')
    render_put('deploy/nginx/proxy.conf', '/etc/nginx/proxy.conf')
...
def apache2_config():
    ...
    render_put('deploy/apache/main', '/etc/apache2/sites-available/main')
    render_put('deploy/apache/sites', '/etc/apache2/sites-available/sites')
    sudo('a2ensite main; a2ensite sites;', pty=True)
    render_put('deploy/apache/apache2.conf', '/etc/apache2/apache2.conf')
    render_put('deploy/apache/ports.conf', '/etc/apache2/ports.conf')
    render_put('deploy/apache/security', '/etc/apache2/conf.d/security')
